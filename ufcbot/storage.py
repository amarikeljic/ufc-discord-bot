...
class Storage:
    ...
    async def pickem_event_name(self, guild_id: int, espn_event_id: str) -> str | None:
        ...
        Read from the picks rather than from ESPN so a leaderboard for a card
        that has been and gone still has a name to show without a request.
        """
        async with self.db.execute(
            "SELECT event_name FROM pickem_picks WHERE guild_id = ? AND espn_event_id = ? LIMIT 1",
            (guild_id, espn_event_id),
        ) as cursor:
            row = await cursor.fetchone()
